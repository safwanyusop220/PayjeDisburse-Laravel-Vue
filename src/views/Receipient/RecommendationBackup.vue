<template>
	<CardCodeExample ref="card">
		<n-space vertical :size="12">
			...
			<n-data-table ref="dataTableInst" :columns="columns" :data="recommendations" :pagination="pagination"
				:row-key="rowKey"
				:max-height="400"
...
export default defineComponent({
  ...
  setup() {
	...
	const getRecommendations = async () => {
		try {
			const url = import.meta.env.VITE_BACKEND_URL +'/api/recipient-program/recommendation'
			const response = await axios.get(url,  { headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` } })
			recommendations.value = response.data.recommendations
			console.log("data>>", recommendations.value)
		} catch (error) {
			console.error(error)
		}
	}
    ...
    return {
		...
		try {
				...
				axios.put(import.meta.env.VITE_BACKEND_URL +'/api/receipients/singleRecommendation', { receipientId, userId }, { headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` } })
				.then((response) => {
        ...
        singleConfirmReject() {
			...
			try {
				...
				Swal.fire({
					...
					if (isConfirmed && text) {
						axios.put(`${import.meta.env.VITE_BACKEND_URL}/api/receipients/singleRejectSubmitted`, { receipientId, text, userId}, {
        ...
        bulkConfirmApprove() {
            try {
                ...
                if (checkedIDs.length > 0) {
                    axios.put(import.meta.env.VITE_BACKEND_URL +'/api/recipient-program/bulkApproveRecommendation', { checkedIDs, userId }, { headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` } })
                    .then((response) => {
                        console.log('Update Status Response:', response.data);
