...
import {NSpace, NButton, NDataTable, NModal, NCard, NForm, NFormItem, NInput, NInputNumber, NGrid, NGi, useMessage, NIcon, NSelect, NDynamicInput, NRadio, NRadioGroup,} from "naive-ui"
  ...
  setup() {
    ...
    return {
		...
		try {
				...
				axios.put(import.meta.env.VITE_BACKEND_URL +'/api/receipients/singleRecommendation', { receipientId })
				.then((response) => {
        ...
        onPositiveClick() {
            try {
                ...
                if (checkedIDs.length > 0) {
                    axios.put(import.meta.env.VITE_BACKEND_URL +'/api/receipients/endorse', { checkedIDs })
                    .then((response) => {
                        console.log('Update Status Response:', response.data);
